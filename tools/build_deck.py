...
def build() -> Path:
    ...
    text(s, Inches(5.3), Inches(4.5), Inches(7.3), Inches(2.1),
         [[("새 시설을 짓지 않습니다.", {"size": 17, "bold": True})],
          [("직매장 12곳은 경쟁자가 아니라 인프라입니다. ON-FARM 은 직매장에 갈 수 없는 "
            "농가를 맡습니다.", {"size": 14, "color": MUTED})],
          [("수요를 데려온다고 말하지 않습니다. ", {"size": 14.5, "color": MUTED}),
           ("농가의 물건이 '올릴 수 있는 상태'가 되게", {"size": 14.5, "bold": True, "color": ORANGE}),
           (" 할 뿐입니다.", {"size": 14.5, "color": MUTED})]],
         spacing=1.4)
    source(s, "쇼핑몰을 새로 만들지 않는다. 지자체 자사몰이 안 되는 이유는 상품이 없어서가 아니라 소비자가 쿠팡에서 사기 때문이다(현장 청취, docs/FIELD_NOTES.md). 수요가 붙는지는 8주 실증의 판매 성사율로 잰다.")
